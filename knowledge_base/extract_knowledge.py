...
EXTRACTION_PROMPT = """You are a financial knowledge engineer. Extract trading concepts, patterns, indicators, strategies, and decision-making rules from the text.

IMPORTANT:
- If the text contains NO trading/finance/investing content, return: {{"concepts": []}}
- Only extract concepts that are clearly defined in the text
- Return an empty array if nothing relevant is found

For each concept, output JSON with:
- concept_name: Short label (3-5 words)
- category: One of ["Technical Pattern", "Indicator", "Fundamental Metric", "Options Strategy", "Risk Rule", "Market Bias", "Candlestick Pattern", "Trading Psychology", "Investment Principle"]
- description: Concise 1-2 sentence definition
- interpretation: What it signals (bullish/bearish/neutral/reversal/continuation)
- conditions: Precise criteria
- market_context: When it works best
...
Output ONLY valid JSON with a "concepts" array. No commentary. Use this exact format:
